...
export function generateCallInConditionWithParam(val: string, param: string): string {
...
export function generateWhileLoop(body: string[], cond: string, inv: boolean): string {
...
export function generateWhileEndLoop(body: string[], cond: string, inv: boolean, bis: boolean): string {
    return `do{${body.join("")}let n;for(n of ${cond}){yield n;}if(${inv !== bis ? "" : "!"}n){break;}}while(true);`;
}

export function generateIfElse(body: string[], elseBody: string[], cond: string, inv: boolean) {
...
export function generateSub(body: string[], subName: string): DefinitionCompilationResult {
    return {
